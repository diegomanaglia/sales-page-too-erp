...
import {
  ...
  Smartphone, Clock, Zap, MessageSquare,
  Award, Lock, PhoneCall, DollarSign,
  AlertTriangle, Wrench, Play, X, Gauge,
  RefreshCw, Layers, Star
} from 'lucide-react';

import { C, GLOBAL_CSS } from './tokens';
import { Badge, CTABtn, ScreenMock, FeatureCard, TestCard, PriceCard, FAQItem } from './Lp3Components';
import FadeIn from '@/components/shared/FadeIn';
import StatNumber from '@/components/shared/StatNumber';
import Countdown from '@/components/shared/Countdown';

const statColors = { value: C.brand, label: C.textMuted };
const countdownColors = { value: C.brand, label: C.textMuted, separator: C.brand };
...
const tabs = [
  {
    ...
    mock: { title: "Ordens de Serviço — Hoje", items: [
  ...
  },
  {
    ...
    mock: { title: "Estoque — Alertas Ativos", items: [
  ...
  },
  {
    ...
    mock: { title: "Financeiro — Resumo Mensal", items: [
  ...
  },
  {
    ...
    mock: { title: "Clientes — Lembretes da Semana", items: [
...
export default function Lp3Page() {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: "'Plus Jakarta Sans',sans-serif", overflowX: "hidden" }}>
      ...
      {/* ANNOUNCEMENT BAR */}
      <div style={{ background: C.brand, overflow: "hidden", height: 36, display: "flex", alignItems: "center" }}>
      ...
      <nav style={{
        ...
        transition: "all 0.35s ease", padding: "0 24px", marginTop: 36,
      }}>
        <div style={{ ...W, display: "flex", justifyContent: "space-between", alignItems: "center", height: 68 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Image src="/logo-sq.png" alt="A1 ERP" width={34} height={34} style={{ objectFit: "contain" }} />
          ...
          <div className="hide-sm" style={{ display: "flex", gap: 32, alignItems: "center" }}>
            {[["#recursos", "Recursos"], ["#depoimentos", "Depoimentos"], ["#planos", "Planos"], ["#faq", "FAQ"]].map(([href, label]) => (
              <a key={href} href={href} style={{ color: C.textMuted, textDecoration: "none", fontSize: 14, fontWeight: 500, transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = C.text}
                onMouseLeave={e => e.target.style.color = C.textMuted}
              >{label}</a>
            ))}
          </div>
          <div className="hide-sm" style={{ display: "flex", gap: 12, alignItems: "center" }}>
      ...
      <header style={{ position: "relative", paddingTop: 180, paddingBottom: 100, overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -100, right: "5%", width: 700, height: 700, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,197,24,0.12) 0%, transparent 65%)", animation: "orb-a 14s ease-in-out infinite", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -200, left: "-5%", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,197,24,0.07) 0%, transparent 65%)", animation: "orb-b 18s ease-in-out infinite", pointerEvents: "none" }} />
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", backgroundImage: `linear-gradient(${C.border} 1px, transparent 1px), linear-gradient(90deg, ${C.border} 1px, transparent 1px)`, backgroundSize: "72px 72px", maskImage: "radial-gradient(ellipse 80% 60% at 50% 0%, black 30%, transparent 100%)", WebkitMaskImage: "radial-gradient(ellipse 80% 60% at 50% 0%, black 30%, transparent 100%)" }} />

        <div style={W}>
          <FadeIn style={{ textAlign: "center" }}>
            <Badge icon={Zap}>Pré-venda · 50 vagas com 40% off</Badge>
            <h1 style={{ fontFamily: "'Outfit'", fontWeight: 900, fontSize: "clamp(36px, 5.5vw, 68px)", lineHeight: 1.06, letterSpacing: -2, margin: "28px auto 0", maxWidth: 860 }}>
              Sua oficina de troca de óleo{" "}
              <span style={{ background: `linear-gradient(90deg, ${C.brand} 0%, ${C.brandLight} 60%, ${C.brand} 100%)`, backgroundSize: "200%", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                no controle total.
              </span>
            </h1>
            <p style={{ fontSize: "clamp(16px, 2vw, 19px)", color: C.textMuted, lineHeight: 1.75, margin: "24px auto 0", maxWidth: 580 }}>
          ...
          <FadeIn delay={0.32} style={{ marginTop: 72, maxWidth: 860, margin: "72px auto 0" }}>
            <div style={{ borderRadius: 18, overflow: "hidden", border: `1px solid ${C.borderBrand}`, boxShadow: `0 0 0 1px ${C.border}, 0 40px 100px rgba(0,0,0,0.6), 0 0 80px ${C.brandGlow}`, background: C.card }}>
              ...
              <div style={{ padding: 24, display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
                {[
                  ...
                  <div key={i} style={{ background: C.card2, borderRadius: 12, padding: "16px 14px", border: `1px solid ${C.border}` }}>
          ...
          <FadeIn delay={0.45} style={{ marginTop: 72, display: "flex", justifyContent: "center", gap: 64, flexWrap: "wrap" }}>
            ...
            <StatNumber end={40} suffix="%" label="OFF vitalício para os primeiros" colors={statColors} />
          </FadeIn>
        </div>
      </header>
        ...
        <div style={W}>
          <FadeIn style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Outfit'", fontSize: "clamp(28px,3.5vw,44px)", fontWeight: 800, margin: "20px 0 16px", lineHeight: 1.2 }}>
              Você administra sua oficina no{" "}<span style={{ color: C.red }}>escuro</span>.
            </h2>
            ...
            {painPoints.map(({ icon: Icon, title, desc }, i) => (
              <FadeIn key={i} delay={i * 0.08}>
                <div style={{ background: C.card, borderRadius: 14, padding: 28, border: `1px solid ${C.border}` }}>
                  <div style={{ width: 40, height: 40, borderRadius: 9, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: C.red, marginBottom: 16 }}>
          ...
          <FadeIn delay={0.4} style={{ textAlign: "center", marginTop: 52 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 12, background: C.brandGlow2, border: `1px solid ${C.borderBrand}`, borderRadius: 12, padding: "14px 24px" }}>
      ...
      <section id="recursos" style={{ ...S(), background: `linear-gradient(180deg, ${C.brandGlow3} 0%, transparent 60%)` }}>
        <div style={W}>
          <FadeIn style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontFamily: "'Outfit'", fontSize: "clamp(28px,3.5vw,44px)", fontWeight: 800, margin: "20px 0 16px", lineHeight: 1.2 }}>
              Tudo que sua oficina precisa.{" "}<span style={{ color: C.brand }}>Nada que não precisa.</span>
            </h2>
          ...
          {tabs.map((tab, i) => i === activeTab && (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }} className="grid-1-md">
              <FadeIn style={{ paddingTop: 8 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 100, background: C.brandGlow2, border: `1px solid ${C.borderBrand}`, color: C.brand, fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", fontFamily: "'Outfit'", marginBottom: 20 }}>
                  <tab.icon size={12} strokeWidth={2.5} /> Módulo — {tab.label}
                </div>
                <h3 style={{ fontFamily: "'Outfit'", fontSize: "clamp(22px,2.5vw,30px)", fontWeight: 800, lineHeight: 1.25, marginBottom: 14 }}>{tab.title}</h3>
              ...
              <FadeIn delay={0.15}><ScreenMock title={tab.mock.title} items={tab.mock.items} /></FadeIn>
            </div>
          ))}
          ...
          <FadeIn style={{ marginTop: 80 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
              {[
                ...
                <FadeIn key={i} delay={i * 0.07}><FeatureCard icon={f.icon} title={f.title} desc={f.desc} /></FadeIn>
              ))}
            </div>
          </FadeIn>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 20, position: "relative" }}>
            {[
              ...
              <FadeIn key={i} delay={i * 0.12}>
                <div style={{ background: C.card, borderRadius: 14, padding: 28, border: `1px solid ${C.border}`, position: "relative" }}>
                  <div style={{ fontFamily: "'Outfit'", fontSize: 56, fontWeight: 900, lineHeight: 1, color: C.brandGlow, position: "absolute", top: 20, right: 20, userSelect: "none" }}>{step}</div>
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: C.brandGlow2, border: `1px solid ${C.borderBrand}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.brand, marginBottom: 18 }}>
      ...
      <section style={{ ...S({ padding: "80px 0" }), background: `linear-gradient(180deg, transparent, ${C.brandGlow3} 50%, transparent)` }}>
        <div style={W}>
          ...
          <FadeIn delay={0.1} style={{ display: "flex", justifyContent: "center" }}>
            <div style={{ background: C.card, borderRadius: 20, padding: "40px 48px", border: `1px solid ${C.borderBrand}`, boxShadow: `0 0 60px ${C.brandGlow}`, maxWidth: 560, width: "100%" }}>
              ...
              <div style={{ marginTop: 36, display: "flex", justifyContent: "center" }}><CTABtn>Quero garantir 40% OFF vitalício</CTABtn></div>
              <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 20, flexWrap: "wrap" }}>
                {[[Shield, "Sem fidelidade"], [RefreshCw, "Cancele quando quiser"], [PhoneCall, "Suporte humanizado"]].map(([Icon, txt], i) => (
          ...
          <FadeIn delay={0.2} style={{ maxWidth: 420, margin: "40px auto 0" }}>
            ...
            <div style={{ height: 8, borderRadius: 100, background: C.card2, overflow: "hidden" }}>
              <div style={{ width: "76%", height: "100%", borderRadius: 100, background: `linear-gradient(90deg, ${C.brandDark}, ${C.brand})`, animation: "blink 2.5s ease-in-out infinite" }} />
            </div>
      ...
      <section id="depoimentos" style={S()}>
        <div style={W}>
          ...
          <FadeIn delay={0.1} style={{ maxWidth: 640, margin: "0 auto 56px" }}>
            <div style={{ background: C.card, borderRadius: 14, overflow: "hidden", border: `1px solid ${C.borderBrand}`, boxShadow: `0 0 40px ${C.brandGlow}`, aspectRatio: "16/9", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
              <div style={{ position: "absolute", inset: 0, background: `radial-gradient(circle at center, ${C.brandGlow2} 0%, transparent 70%)` }} />
              <div style={{ width: 72, height: 72, borderRadius: "50%", background: C.brand, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 40px ${C.brandGlow}` }}>
                <Play size={28} strokeWidth={0} fill={C.btnText} style={{ marginLeft: 4 }} />
              </div>
              <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.7)", borderRadius: 8, padding: "8px 16px", backdropFilter: "blur(8px)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 20 }}>
            <FadeIn><TestCard initials="RM" name="Ricardo Mendes" role="Oficina Rápida — Osasco, SP" text="Antes eu perdia tempo anotando tudo no caderno e ainda assim não sabia quanto lucrava. Com o A1 ERP fechei o mês passado sabendo exatamente: R$ 14.200 de receita e R$ 8.900 de lucro. Isso mudou tudo." /></FadeIn>
            <FadeIn delay={0.1}><TestCard initials="CL" name="Carlos Lima" role="Auto Center Express — Barueri, SP" text="Meu funcionário aprendeu a usar em literalmente 10 minutos. O envio da OS por WhatsApp é um diferencial absurdo — cliente adora receber o comprovante na hora. Profissionaliza muito a imagem da oficina." /></FadeIn>
            <FadeIn delay={0.2}><TestCard initials="AF" name="Ana Ferreira" role="Troca Certa — Campinas, SP" text="Finalmente um sistema que entende que troca de óleo é um negócio específico. Não é um ERP genérico adaptado. Foi feito pra gente — e dá pra sentir isso em cada detalhe do sistema." /></FadeIn>
          </div>
        </div>
      </section>
      ...
      <section style={{ ...S(), background: `linear-gradient(180deg, ${C.brandGlow3} 0%, transparent 60%)` }}>
        <div style={W}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }} className="grid-1-md">
            <FadeIn>
              <div style={{ borderRadius: 16, overflow: "hidden", aspectRatio: "1/1", background: `linear-gradient(135deg, ${C.card2} 0%, ${C.card} 100%)`, border: `1px solid ${C.borderBrand}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={W}>
          <FadeIn style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Outfit'", fontSize: "clamp(28px,3.5vw,44px)", fontWeight: 800, margin: "20px 0 12px" }}>
              Investimento que se paga{" "}<span style={{ color: C.brand }}>na primeira semana.</span>
            </h2>
          ...
          <div style={{ display: "flex", gap: 20, justifyContent: "center", flexWrap: "wrap", alignItems: "flex-start" }}>
            <FadeIn><PriceCard name="Essencial" price="97" originalPrice="147" features={["Ordem de Serviço digital", "Cadastro de clientes", "Controle de estoque básico", "Acesso mobile", "Suporte por WhatsApp"]} /></FadeIn>
            <FadeIn delay={0.08}><PriceCard name="Profissional" price="147" originalPrice="247" highlight badge="Mais popular" features={["Tudo do Essencial, mais:", "Financeiro completo e relatórios", "KPIs e dashboard executivo", "Notificações automáticas de retorno", "Catálogo de filtros integrado", "Suporte prioritário"]} /></FadeIn>
            <FadeIn delay={0.16}><PriceCard name="Rede" price="247" originalPrice="397" features={["Tudo do Profissional, mais:", "Multi-unidades ilimitadas", "Gestão de equipe e permissões", "API de integração", "Onboarding dedicado", "Consultoria de implantação"]} /></FadeIn>
          </div>
          <FadeIn delay={0.25} style={{ textAlign: "center", marginTop: 36 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "center", background: C.brandGlow2, border: `1px solid ${C.borderBrand}`, borderRadius: 10, padding: "10px 20px", fontSize: 13, color: C.textMuted }}>
      ...
      <section style={S({ padding: "80px 0" })}>
        <div style={W}>
          <FadeIn>
            <div style={{ background: `linear-gradient(135deg, ${C.card} 0%, rgba(245,197,24,0.04) 100%)`, borderRadius: 20, padding: "48px 52px", border: `1px solid ${C.borderBrand}`, display: "flex", gap: 40, alignItems: "center", flexWrap: "wrap", boxShadow: "0 0 60px rgba(245,197,24,0.08)" }}>
              <div style={{ width: 108, height: 108, borderRadius: "50%", flexShrink: 0, background: C.brandGlow2, border: `2px solid ${C.brand}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <section id="faq" style={S()}>
        <div style={{ ...W, maxWidth: 700 }}>
          <FadeIn style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontFamily: "'Outfit'", fontSize: "clamp(26px,3.5vw,40px)", fontWeight: 800, margin: "20px 0" }}>Ainda tem dúvidas?</h2>
          </FadeIn>
      ...
      <section style={{ ...S(), textAlign: "center", background: `linear-gradient(180deg, transparent, ${C.brandGlow3} 50%, transparent)`, position: "relative" }}>
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 600, height: 400, background: `radial-gradient(ellipse, ${C.brandGlow} 0%, transparent 70%)`, pointerEvents: "none" }} />
        <div style={W}>
          <FadeIn>
            <h2 style={{ fontFamily: "'Outfit'", fontSize: "clamp(28px,4.5vw,52px)", fontWeight: 900, lineHeight: 1.12, marginBottom: 20, maxWidth: 700, margin: "0 auto 20px" }}>
              ...
              <span style={{ color: C.brand }}>não.</span> Merece crescer.
            </h2>
            ...
            <div style={{ marginTop: 24, display: "flex", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
              {[[Lock, "Pagamento seguro"], [Shield, "7 dias de garantia"], [PhoneCall, "Suporte via WhatsApp"]].map(([Icon, txt], i) => (
      ...
      <footer style={{ padding: "48px 0", borderTop: `1px solid ${C.border}` }}>
        <div style={{ ...W, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Image src="/logo-sq.png" alt="A1 ERP" width={28} height={28} style={{ objectFit: "contain" }} />
            <span style={{ fontFamily: "'Outfit'", fontWeight: 600, fontSize: 14, color: C.textMuted }}>A1 ERP</span>
          </div>
          ...
          <div style={{ fontSize: 13, color: C.textSubtle }}>&copy; 2026 A1 ERP. Todos os direitos reservados.</div>
        </div>
      </footer>
    </div>
  );
